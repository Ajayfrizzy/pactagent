...
`failed` is deliberately scope-blocking: it records an unresolved settlement
outcome that requires reconciliation, not a resolved failure after which a new
escrow is safe. Integrators must not create a replacement escrow while either
`failed` or `reconciliation_required` is present. The resolved outcomes are
`released` and `refunded`; CKB reorg observation may move either back to
...
| Server unit/e2e tests | Pass, 95 passed and 0 failed |
| PostgreSQL/Redis integration | Pass, 38 passed and 0 failed against fresh migrated services |
...
| CKB public testnet RPC smoke | Not rerun; 2026-08-28 non-mutating pass retained, lifecycle/deployment not exercised |

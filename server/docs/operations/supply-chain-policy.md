...
Current accepted findings (reviewed 2026-07-22): `GHSA-848j-6mx2-7j84` remains transitive through `@ckb-ccc/core`/JoyID/`elliptic`; the suggested automated fix is an incompatible downgrade and must wait for upstream plus the compatibility suite. `GHSA-f88m-g3jw-g9cj` affects the Sharp line currently supported by Next; image optimization is disabled and the web Docker build removes Sharp/libvips from the runtime image. Both exceptions expire on 2026-08-05 and must be closed or explicitly renewed after upstream review. `GHSA-92pp-h63x-v22m` is in Prisma development tooling and is not shipped in the runtime image.

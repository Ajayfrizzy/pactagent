...
Current accepted finding (reviewed 2026-08-25): Prisma 7.9.x still pins `deepmerge-ts@7.1.5` in the root development workspace. The standalone server lock and migration image force `deepmerge-ts@8.0.0`, so the affected version is not shipped. The exact high-severity advisory exception expires on 2026-09-22 and must be removed when Prisma adopts the patched major line or explicitly renewed after review. Patched `fast-uri`, `js-yaml`, `nanoid`, and `valibot` versions are also enforced through workspace or standalone server overrides.

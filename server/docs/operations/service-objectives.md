...
| Settlement reconciliation | Successful reconciliations / attempts | 99.9% | 30 days |
| Webhook delivery | Delivered within configured attempts | 99% | 7 days |
| Worker continuity | Fresh settlement heartbeat within 60 seconds | 99.9% | 30 days |

Page immediately for worker loss, database saturation, provider circuit opening on settlement dependencies, repeated reconciliation failure, or a sustained API error budget burn. Ticket non-urgent webhook/provider degradation and WebSocket pressure. Dead letters always require ownership and disposition.

Prometheus rules in `observability/prometheus/alerts.yaml` encode the initial thresholds. Grafana dashboards in `observability/dashboards` provide API, worker, queue, settlement, webhook, database, and provider views. Every alert must link to an owner and runbook in the external alert manager before production notification routing is enabled.
